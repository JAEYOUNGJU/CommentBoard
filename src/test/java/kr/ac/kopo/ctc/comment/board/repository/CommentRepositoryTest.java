...
public class CommentRepositoryTest {
	...
	void createContent() {
		...
		comment.setContent("주재영");
		comment.setTitle("재영");
		comment.setRelevel(1);
		comment.setViewcnt(0);
	...
	@Test
	void update() {
		Optional<Comment> comment = commentRepository.findById(1); //Optional 객체 생성
		...
		comment.ifPresent(selectComment -> { //null을 확인하는 if문 대신 ifPresent 함수 호출
			selectComment.setDate(new Date());
			selectComment.setTitle("댓글수정");
			selectComment.setContent("댓글 내용 수정");

			commentRepository.save(selectComment); //DB에 insert 한다.
		});
	}
	// 3.selectOne
	@Test
	void selectOne() {
		Optional<Comment> comment = commentRepository.findOneById(1); //id 중 Long 타입의 1 선택
	}
//	// 4.selectAll
//	@Test
//	void selectAll() {
